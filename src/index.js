...
async function connectToWhatsApp() {
    ...
    sock.ev.on('connection.update', async (update) => {
        ...
        // When socket negotiates keys, grab the pairing code immediately!
        if (qr && !sock.authState.creds.registered) {
            if (!process.env.WHATSAPP_NUMBER) {
                ...
                try {
                    // Slight breather to ensure socket is ready
                    setTimeout(async () => {
                ...
                } catch(e) { console.error("Pairing Error:", e.message || e) }
            }
        }
    ...
    sock.ev.on('messages.upsert', async m => {
        ...
        if (textMessage) {
            ...
            // 2) User asks for details of a serial number (e.g. 1, 2)
            ...
            // 3/4) User asks for specific date (DD-MM-YYYY) or general "Anything"
            else {
                ...
                if (events.length === 0) {
                    ...
                    updateSession(senderId, events); // Save to RAM for serial no checks
                }
            }

            try {
                // Emulate human typing speed dynamically based on message length
                await sock.sendPresenceUpdate('composing', senderId);
                const typingTimeMs = Math.min(3000, Math.max(1000, replyText.length * 10)); // Between 1 and 3 seconds
                await new Promise(res => setTimeout(res, typingTimeMs));
                await sock.sendPresenceUpdate('paused', senderId);
